...
export const createOnboardingColumns = ({ onEdit, onDelete }) => [
  {
    id: "select",
    header: ({ table }) => (
      <input
...

  {
    ...
    header: ({ column }) => <SortHeader column={column} title="Joining Date" />,
...

  {
    ...
    cell: ({ row }) => {
      ...
      const submitted = Object.values(docs).filter(
        (doc) => doc.submitted,
      ).length;
